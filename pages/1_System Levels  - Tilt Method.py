...
with col1:
    ...
    LFL = st.number_input("Enter Level at Carrier Frequency (dBmV)", key="LFL2", value=52, step=step_size_ch)
    SPLIT = st.selectbox("Select Split", options=["Low", "Mid", "High"], key="SPLIT2")
    Calc = st.button("Show Plot", key='calculate2', on_click=plot_levels, type="primary")
...
try:
    st.info(
        f"Level at {FIND_FQ} MHz is:     :green[{round(fu.mystery_freq(float(HF), float(HFL), float(LF), float(LFL), float(FIND_FQ), SPLIT)[1], digits)}] dBmV")
except ValueError:
    pass
...
try:
    tilt = fu.mystery_freq(float(HF), float(HFL), float(LF), float(LFL), float(FIND_HP), SPLIT)[1] - \
           fu.mystery_freq(float(HF), float(HFL), float(LF), float(LFL), float(FIND_LP), SPLIT)[1]
